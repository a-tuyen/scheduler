...
describe("Application", () => {
  ...
  it("loads data, cancels an interview and increases the spots remaining for Monday by 1", async () => {
    ...
    // 3. Click the "cancel" button on the "Archie Cohen" "appointment.
    ...
    // 8. Check that DayListItem with the text "Monday" also has the text "2 spots reamining"
    const day = getAllByTestId(container, "day").find(day => 
      queryByText(day, "Monday")
    );
  ...
  it("loads data, edits an interview and keeps the spots remaining for Monday the same", async () => {
    ...
    // 4. Enter the name "Lydia Miller-Jones" into the input with the placeholder "Archie Cohen".
    fireEvent.change(getByPlaceholderText(appointment, /enter student name/i), {
      target: { value: "Lydia Miller-Jones" } 
    });
